...
#%matplotlib inline
spark = SparkSession.builder.appName('ipl_bowl_cluster').getOrCreate()
data = spark.read.csv('bowl_profile_ipl.csv', header=True, inferSchema=True)
#data = spark.read.csv('hdfs://master:9000/inputdata/bat_profile_ipl.csv')
data.printSchema()
data.show(20)
print(type(data))
data2 = data.select('PID','Name','Wickets','Econ','Avg','SR')
data2.show()
cols = ['Wickets','Econ','Avg','SR']
...
#From the plot, we can see that optimal k is 7. In this example, we will consider 7
k_means_bat = KMeans(featuresCol='features', k=7)
...
#KMeans_bat_fitted.toPandas().to_csv('IPL_Bowl_Cluster.csv')